...
pub fn get_message(message_id: &str) -> String {
    match message_id {
        ...
        "err-mixed-recipient-passphrase" => "Cannot combine passphrase encryption with other recipients".to_string(),
        "err-incompatible-recipients-oneway" => format!("Recipients with labels {} are incompatible with other recipients", "{labels}"),
        "err-incompatible-recipients-twoway" => format!("Recipients with labels {} are incompatible with recipients with labels {}", "{left}", "{right}"),
        ...
        "rec-unknown-format" => "This file was created by a newer version of age that we do not support.".to_string(),
        "rec-excessive-work" => format!("This file would take {} seconds to decrypt on this device.", "{duration}"),
        "rec-missing-plugin" => "You may need to install additional software to decrypt this file.".to_string(),
        "err-identity-file-contains-plugin" => format!("Identity file {} contains a plugin identity ({})", "{filename}", "{plugin_name}"),
        "rec-identity-file-contains-plugin" => format!("The {} plugin does not support converting identities to recipients.", "{plugin_name}"),
        ...
        "err-plugin-recipient" => format!("Plugin {} recipient {} error: {}", "{plugin_name}", "{recipient}", "{message}"),
        "err-excessive-work" => "This passphrase is very expensive to decrypt.".to_string(),
        "encrypted-warn-no-match" => format!("Warning: could not decrypt identity in {}", "{filename}"),
...
pub fn get_message_with_args<T: std::fmt::Display>(message_id: &str, args: &[T]) -> String {
    ...
    if !args.is_empty() {
        ...
        for (i, arg) in arg_strings.iter().enumerate() {
            match i {
                0 => msg = msg.replace("{err}", arg).replace("{filename}", arg).replace("{plugin_name}", arg).replace("{labels}", arg).replace("{left}", arg).replace("{duration}", arg),
...
/// Helper function for backward compatibility
#[doc(hidden)]
pub(crate) fn error_string_with_args<T: std::fmt::Display>(message_id: &str, args: &[(&str, T)]) -> String {
    let mut msg = get_message(message_id);
    for (name, value) in args {
